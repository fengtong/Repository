jQuery.extend({
  ...
  ajaxFileUpload:function(s){
    ...
    if(typeof(s.data)=='string'){
      s.data=jQuery._paramToJson(s.data);
    }
    var form=jQuery.createUploadForm(id,s.fileElementId,(typeof(s.data)=='undefined'?false:s.data));
    var frame=jQuery.createUploadIframe(id,s.secureuri);
  ...
  /**
   * transfer http params to json object, the params should get by
   * $('#formid').serialize(). this function support checkbox params submit,
   * eg.: id=1&id=2
   */
  _paramToJson:function(param){
    var array=param.split('&');
    var result={};
    for(var i=0;i<array.length;i++){
      var index=array[i].indexOf('=');
      if(result[array[i].substring(0,index)])
        result[array[i].substring(0,index)]=result[array[i].substring(0,index)]+","
            +array[i].substring(index+1);
      else
